...
namespace fourg2.classes
{
    public class config : banco
    {
        ...
        public int DeletaAnotacao()
        {
            ...
            if (VerificaAnotacao())
            {
                string y = "DELETE FROM fg.anotacao_usuario WHERE id_usuario=?id_usuario;";
                resultado = ExecuteNonQuery(y,
                new MySqlParameter("id_usuario", this.idusuario));
            }
            return resultado;
        }
        public bool VerificaPauta()
        {
            bool resultado = false;
            DataTable dt = new DataTable();
            string x = "Select * from fg.pauta where id_usu=?id_usuario;";
            MySqlDataAdapter DrrDados = Adapter(x,
                new MySqlParameter("id_usuario", this.idusuario));
            if (DrrDados != null)
            {
        ...
        public int DeletarPauta()
        {
            ...
            {
                resultado = -1;
                string y = "DELETE FROM fg.pauta WHERE id_usu=?id_usuario;";
                resultado = ExecuteNonQuery(y,
                    new MySqlParameter("id_usuario", this.idusuario));
            }
            return resultado;
        }
        public bool VerificaMateria()
        {
            bool resultado = false;
            DataTable dt = new DataTable();
        ...
        public int DeletaMateria()
        {
            ...
            if (VerificaMateria())
            {
                string y = "DELETE FROM fg.cad_materia WHERE id_usuario=?id_usuario;";
                resultado = ExecuteNonQuery(y,
                    new MySqlParameter("id_usuario", this.idusuario));
            }
            return resultado;
